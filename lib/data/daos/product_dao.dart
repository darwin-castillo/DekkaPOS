...
class ProductDao {
  ...
  Future<entity.Product?> getById(int id) async {
    final result = await (_db.select(_db.products)..where((t) => t.id.equals(id)))
        .getSingleOrNull();
  ...
  Future<List<entity.Product>> getByCategory(int idCategory) async {
    final results = await (_db.select(_db.products)
          ..where((t) => t.idCategory.equals(idCategory)))
        .get();
    return results.map(_productFromRow).toList();
  }
  ...
  Future<List<entity.Product>> getByProvider(int idProvider) async {
    final results = await (_db.select(_db.products)
          ..where((t) => t.idProvider.equals(idProvider)))
        .get();
    return results.map(_productFromRow).toList();
  }

  Future<int> update(entity.Product product) async {
    return await (_db.update(_db.products)
